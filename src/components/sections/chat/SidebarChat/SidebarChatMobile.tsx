...
export default function SidebarChatMobile({
  selectedChat,
  setSelectedChat,
  isOpen,
  setIsOpen,
}: SidebarChatMobileProps) {
  const chats = [
    {
      username: "Alice",
      avatarUrl:
        "https://static.todamateria.com.br/upload/57/37/573765cb43fa8-baruch-spinoza.jpg",
  ...
  return (
    <>
      ...
      <aside
        className={`${styles.sidebar_mobile} ${isOpen ? styles.open : ""}`}
      >
            ...
            {chats.map((chat, i) => (
              <li
                ...
                className={`${styles.chat_item} ${
                  selectedChat === i ? styles.selected : ""
                }`}
                onClick={() => {
                  setSelectedChat(i);
                  setIsOpen(false);
